...
// v7 = v6 visual, but every label lives in HTML instead of being baked into the JPG
const baseSrc = "public/brand/generated/hero-promo-v3-base.jpg";
const faceSrc = "public/brand/generated/rppg-face-scan-green.png";
const outLive = "public/brand/hero-promo-v7.jpg";
const outArchive = "public/brand/generated/hero-promo-rppg-v7.jpg";
...
// The top band of the base art carries the old "See your / Heart Pattern" copy,
// so it stays covered by a gradient and is cropped away at the end.
const TOP_CROP = 250;
const BOTTOM_CROP = 110;
...
<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">
  ...
  <rect x="0" y="${H - 260}" width="${W}" height="120" fill="url(#footFade)"/>
  <rect x="0" y="${H - 140}" width="${W}" height="140" fill="url(#footSolid)"/>
  <rect x="${W / 2 - 140}" y="${H - 62}" width="280" height="3" rx="1.5" fill="#e8919a" fill-opacity="0.45"/>
</svg>`);
...
const croppedH = H - TOP_CROP - BOTTOM_CROP;
const cropped = await sharp(composed)
  .extract({ left: 0, top: TOP_CROP, width: W, height: croppedH })
  .jpeg({ quality: 88, mozjpeg: true })
  .toBuffer();

await sharp(cropped).toFile(outLive);
await sharp(cropped).toFile(outArchive);

// The HTML labels are anchored to the face badge, so publish its geometry here.
console.log("OK", {
  outLive,
  ...
  height: croppedH,
  facePct: {
    left: ((faceLeft / W) * 100).toFixed(2),
    top: (((faceTop - TOP_CROP) / croppedH) * 100).toFixed(2),
    size: ((faceSize / W) * 100).toFixed(2),
    bottom: (((faceTop - TOP_CROP + faceSize) / croppedH) * 100).toFixed(2),
  },
});
